...
var detection_radius: float = 100.0  # Radius to detect other NPCs
...
func detect_npcs_around():
	var npc_position = get_parent().global_position
	var nearby_npcs = []
	
	# Iterate through all NPCs in the "npcs" group
	for npc in get_tree().get_nodes_in_group("npcs"):
		# Skip self
		if npc == get_parent():
			continue

		# Calculate the distance to the current NPC
		var distance = npc_position.distance_to(npc.global_position)

		# If within detection_radius, add to the list
		if distance <= detection_radius:
			nearby_npcs.append({"npc": npc, "distance": distance})

	# Update detection_status with the detected NPCs
	detection_status["nearby_npcs"] = nearby_npcs.size()
	emit_signal("detection_status_updated", detection_status)
